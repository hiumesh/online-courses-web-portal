...
create type course_section_content_type_enum as enum ('Video', 'Document');
...
create table
  ...
  create table
    public.course_section (
      ...
      constraint course_section_course_id_fkey foreign key (course_id) references courses (id) on delete cascade
    ) tablespace pg_default;

  create table
    public.course_section_content (
      id bigint generated by default as identity not null,
      section_id bigint not null,
      order_index integer not null,
      title text not null,
      type course_section_content_type_enum not null,
      url text not null,
      resources json null,
      created_at timestamp with time zone null default now(),
      constraint course_section_content_pkey primary key (id),
      constraint course_section_content_section_id_fkey foreign key (section_id) references course_section (id) on delete cascade
    ) tablespace pg_default;
...
create or replace view sub_category_topics as
select sub_category.name, topics.name as topic, count(distinct courses.id) as course_count
from 
  sub_category
  left join courses on sub_category.id = courses.sub_category
  join course_topics on course_topics.course_id = courses.id
  join topics on topics.id = course_topics.topic_id
group by
  sub_category.name, topics.name;
