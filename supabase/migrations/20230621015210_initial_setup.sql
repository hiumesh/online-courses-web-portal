...
select courses.id, courses.title, courses.short_description, categories.name as categories, sub_categories.name as sub_categories, courses.avg_rating, courses.language, courses.is_paid, courses.level, json_agg(topics.name) as topic
from
  courses
  join categories on categories.id = courses.categories
  join sub_categories on sub_categories.id = courses.sub_categories
...
select courses.id, courses.image, courses.title, courses.short_description, courses.enrollments_count, courses.created_at, categories.name as categories, sub_categories.name as sub_categories, courses.avg_rating, courses.is_paid, courses.level, prices.amount as amount,  courses.language, courses.review_count, json_agg(distinct tags.name) as tag, json_agg(distinct topics.name) as topic, json_agg(distinct user_profiles.username) as instructor
from
  courses
  join categories on categories.id = courses.categories
  join sub_categories on sub_categories.id = courses.sub_categories
...
from 
  sub_categories
  left join courses on sub_categories.id = courses.sub_categories
  join course_topics on course_topics.course_id = courses.id
  join topics on topics.id = course_topics.topic_id
group by
...
begin
  ...
  values
    (user_id, user_id, user_id, format('{"sub":"%s","email":"%s"}', user_id::text, email)::jsonb, 'email', '2023-05-03 19:41:43.582456+00', '2023-05-03 19:41:43.582497+00', '2023-05-03 19:41:43.582497+00');
    ...
    (
      ...
      from (
        ...
        where
          ...
          AND (prices is null or is_paid in (select unnest(prices)))
    ...
    (
      ...
      from (
        ...
        from 
          courses_sm sm
        where
          (categories is null or categories in (SELECT unnest(categories)))
          AND (sub_categories is null or sub_categories in (select unnest(sub_categories)))
          AND (levels is null or level in (select unnest(levels)))
          AND (rating is null or avg_rating >= rating)
          AND (languages is null or language in (select unnest(languages)))
          AND (prices is null or is_paid in (select unnest(prices)))
...
begin
  ...
  if sort = 'mostpop' then
      ...
      where
          ...
          AND (categories is null or c.categories in (SELECT unnest(categories)))
          AND (sub_categories is null or c.sub_categories in (select unnest(sub_categories)))
          ...
          AND (prices is null or c.is_paid in (select unnest(prices)))
          AND (topics is null or topic::jsonb ?& topics)
      order by
